...
public class RoomServlet extends HttpServlet {
    ...
    private void list(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException, ServletException {
        String stPageNumber = request.getParameter("pageNum");
        int pageNum = stPageNumber != null ? Integer.parseInt(stPageNumber) : 1;
        List<Room> roomList = roomService.findAllWithPaging(pageNum, ConstantUtil.pageSize);
        ...
        int pageTotal = (int) Math.ceil((double) roomService.findAll().size() / ConstantUtil.pageSize);

        List<Integer> range = IntStream.rangeClosed(1, pageTotal)
                .boxed().collect(Collectors.toList());

        request.setAttribute("pageSize", ConstantUtil.pageSize);
